...
- Đặc biệt PHẢI TRÍCH XUẤT:
  ...
  • MỌI số phải được nhận diện dù viết bằng chữ hay bằng chữ số. Ví dụ: "chín" → 9, "mười một" → 11, "hai mươi" → 20, "một trăm" → 100.
...
- Nếu text có số viết bằng chữ, PHẢI ghi nhận và chuyển sang số chữ số tương đương khi tạo properties
...
2. Nếu text có số viết bằng chữ, hãy chuyển sang số chữ số tương đương trong property numeric khi hợp lý
3. Tự do thêm properties BẤT KỲ khi thấy thông tin trong text
4. TẠO TỐI ĐA relationships - kết nối mọi thứ liên quan
5. KHÔNG có node rỗng (phải có description hoặc mô tả tương đương)
6. Sử dụng relationship types từ danh sách được phép
7. {priority_instruction}
...
class CustomGraphExtractor:
    ...
    def _enrich_properties_from_text(self, data: Dict[str, Any], text: str) -> Dict[str, Any]:
        ...
        for node in nodes:
            ...
            for prop_key, prop_value in list(props.items()):
                if isinstance(prop_value, str):
                    normalized = self._parse_vietnamese_number(prop_value)
                    if normalized is not None:
                        props[prop_key] = normalized
    ...
    def _parse_vietnamese_number(self, text: str) -> Optional[int]:
        """Normalize simple Vietnamese number words into digits."""
        ...
        number_map = {
            ...
            "mười": 10, "mười một": 11, "mười hai": 12, "mười ba": 13, "mười bốn": 14,
            "mười lăm": 15, "mười sáu": 16, "mười bảy": 17, "mười tám": 18, "mười chín": 19,
